import React from 'react';

const ServicesMenuIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M6 8C7.10457 8 8 7.10457 8 6C8 4.89543 7.10457 4 6 4C4.89543 4 4 4.89543 4 6C4 7.10457 4.89543 8 6 8Z"
      fill="#8E9AB2"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M12 8C13.1046 8 14 7.10457 14 6C14 4.89543 13.1046 4 12 4C10.8954 4 10 4.89543 10 6C10 7.10457 10.8954 8 12 8Z"
      fill="#8E9AB2"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M18 8C19.1046 8 20 7.10457 20 6C20 4.89543 19.1046 4 18 4C16.8954 4 16 4.89543 16 6C16 7.10457 16.8954 8 18 8Z"
      fill="#8E9AB2"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M6 14C7.10457 14 8 13.1046 8 12C8 10.8954 7.10457 10 6 10C4.89543 10 4 10.8954 4 12C4 13.1046 4.89543 14 6 14Z"
      fill="#8E9AB2"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d={'M12 14C13.1046 14 14 13.1046 14 12C14 10.8954 13.1046 10 12 10C10.8'
        + '954 10 10 10.8954 10 12C10 13.1046 10.8954 14 12 14Z'}
      fill="#8E9AB2"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d={'M18 14C19.1046 14 20 13.1046 20 12C20 10.8954 19.1046 10 18 10C16.8954 10 16 10.8954'
        + ' 16 12C16 13.1046 16.8954 14 18 14Z'}
      fill="#8E9AB2"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M6 20C7.10457 20 8 19.1046 8 18C8 16.8954 7.10457 16 6 16C4.89543 16 4 16.8954 4 18C4 19.1046 4.89543 20 6 20Z"
      fill="#8E9AB2"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d={'M12 20C13.1046 20 14 19.1046 14 18C14 16.8954 13.1046 16 12 16C10.8954 16 10 16.8954'
        + ' 10 18C10 19.1046 10.8954 20 12 20Z'}
      fill="#8E9AB2"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d={'M18 20C19.1046 20 20 19.1046 20 18C20 16.8954 19.1046 16 18 16C16.8954 16 16 16.8954'
        + ' 16 18C16 19.1046 16.8954 20 18 20Z'}
      fill="#8E9AB2"
    />
  </svg>

);

export default ServicesMenuIcon;
